#!/usr/bin/env python3
"""Structural gate for swiftlys2-skills. No toolchain needed.
...
Checks (fail fast with a clear message):
  ...
  5. No C# fundamentals duplicated here: files must not teach nullable,
     binlogs, or test-runner syntax (those live in csharp-skills).
...
# Pure-C# topics that belong in csharp-skills, never here.
FORBIDDEN = ("binlog", "vstest", "nullable reference types", "MTP runner")
...
def main() -> int:
    ...
    for path in sorted(RULES.glob("*.md")):
        ...
        for topic in FORBIDDEN:
            if topic in low:
                fail(
                    f"rules/{path.name}: teaches `{topic}` — "
                    f"belongs in csharp-skills, link there instead"
                )
